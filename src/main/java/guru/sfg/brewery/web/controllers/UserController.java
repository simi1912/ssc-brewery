...
public class UserController{
    ...
    public String confirm2Fa(@RequestParam Integer verifyCode){
        ...
        if(googleAuthenticator.authorizeUser(user.getUsername(), verifyCode)){
            ...
            log.debug("Verify code ok. Forward to index.");

            return "index";
        }else {
            log.debug("Verify code bad. Forward to user/register2fa.");
    ...
    public String verifyPostOf2Fa(@RequestParam Integer verifyCode){
        ...
        log.debug("Posted verify  code: " + verifyCode);

        if(googleAuthenticator.authorizeUser(user.getUsername(), verifyCode)){
            ((User) SecurityContextHolder.getContext().getAuthentication()
                    .getPrincipal()).setGoogle2FaRequired(false);
            log.debug("Verify code authorized. Forward to index.");
            return "/index";
        }else {
            log.debug("Verify code not authorized. Forward to user/verify2fa.");
            return "user/verify2fa";
        }
